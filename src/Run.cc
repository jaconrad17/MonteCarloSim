  ...
  : G4Run()
{
  ...
  // Initialize and resize the vector 'fBoundaryProcs'
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
// Destructor for the Run class
Run::~Run() 
{
  // Empty destructor as there is no dynamic memory allocated to clean up
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
// Set information about the primary particle
void Run::SetPrimary(G4ParticleDefinition* particle, G4double energy, G4bool polarized, G4double polarization)
{
  fParticle = particle;
  fEkin = energy;
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
// Merge information from another run into the current run
void Run::Merge(const G4Run* run)
{
  ...
  // Accumulate+energy and count information
  ...
  // Accumulate total surface, optical absorption, and boundary processes information
  ...
  // Call the base class's Merge method
  G4Run::Merge(run);
}
...
void Run::EndOfRun()
{
  // Check if there were no events, return if so
  if(numberOfEvent == 0)
    return;

  // Calculate hte total number of events as a double
  ...
  id = analysisMan->GetH1Id("All");
  analysisMan->SetH1XAxisTitle(id, "Polar Angle [deg]");
  analysisMan->SetH1YAxisTitle(id, "Number of photons");

  id = analysisMan->GetH1Id("Trans_azim");
  analysisMan->SetH1XAxisTitle(id, "Azimuthal Angle [deg]");
  analysisMan->SetH1YAxisTitle(id, "Number of photons");

  id = analysisMan->GetH1Id("Refl_azim");
  analysisMan->SetH1XAxisTitle(id, "Azimuthal Angle [deg]");
  analysisMan->SetH1YAxisTitle(id, "Number of photons");

  id = analysisMan->GetH1Id("All_azim");
  analysisMan->SetH1XAxisTitle(id, "Azimuthal Angle [deg]");
  analysisMan->SetH1YAxisTitle(id, "Number of photons");

  id = analysisMan->GetH1Id("Time");
  analysisMan->SetH1XAxisTitle(id, "Time of Arrival [ns]");
  analysisMan->SetH1YAxisTitle(id, "Number of photons");

  id = analysisMan->GetH1Id("sLength");
  analysisMan->SetH1XAxisTitle(id, "Step Length [m]");
  analysisMan->SetH1YAxisTitle(id, "Number of photons");

  id = analysisMan->GetH1Id("sNum");
  analysisMan->SetH1XAxisTitle(id, "Step Number");
  analysisMan->SetH1YAxisTitle(id, "Number of photons");

  id = analysisMan->GetH1Id("Polar 2nd peak");
  analysisMan->SetH1XAxisTitle(id, "Polar Angle [deg]");
  analysisMan->SetH1YAxisTitle(id, "Number of photons");

  id = analysisMan->GetH1Id("Azim 2nd peak");
  analysisMan->SetH1XAxisTitle(id, "Azimuthal Angle [deg]");
  analysisMan->SetH1YAxisTitle(id, "Number of photons");

  id = analysisMan->GetH1Id("Reflected_NF");
  analysisMan->SetH1XAxisTitle(id, "Polar Angle [deg]");
  analysisMan->SetH1YAxisTitle(id, "Number of photons");
  ...
  const DetectorConstruction* det = (const DetectorConstruction*) (G4RunManager::GetRunManager()->GetUserDetectorConstruction());
  ...
  G4cout << "\n    Run Summary\n";
  G4cout << "---------------------------------\n";
  G4cout << "Primary particle was: " << fParticle->GetParticleName() << " with energy " << G4BestUnit(fEkin, "Energy") << "." << G4endl;
  ...
  {
    G4cout << "Average energy of Cerenkov photons created per event: " << (fCerenkovEnergy / eV) / TotNbofEvents << " eV." << G4endl;
    G4cout << "Average number of Cerenkov photons created per event: " << fCerenkovCount / TotNbofEvents << G4endl;
    if(fCerenkovCount > 0)
    {
      G4cout << " Average energy per photon: " << (fCerenkovEnergy / eV) / fCerenkovCount << " eV." << G4endl;
    }
    G4cout << "Average energy of scintillation photons created per event: " << (fScintEnergy / eV) / TotNbofEvents << " eV." << G4endl;
    G4cout << "Average number of scintillation photons created per event: " << fScintCount / TotNbofEvents << G4endl;
    if(fScintCount > 0)
    {
      G4cout << " Average energy per photon: " << (fScintEnergy / eV) / fScintCount << " eV." << G4endl;
    }
  }

  G4cout << "Average number of photons absorbed by WLS per event: " << fWLSAbsorptionCount / G4double(TotNbofEvents) << " " << G4endl;
  if(fWLSAbsorptionCount > 0)
  {
    G4cout << " Average energy per photon: " << (fWLSAbsorptionEnergy / eV) / fWLSAbsorptionCount << " eV." << G4endl;
  }
  G4cout << "Average number of photons created by WLS per event: " << fWLSEmissionCount / TotNbofEvents << G4endl;
  if(fWLSEmissionCount > 0)
  {
    G4cout << " Average energy per photon: " << (fWLSEmissionEnergy / eV) / fWLSEmissionCount << " eV." << G4endl;
  }
  G4cout << "Average energy of WLS photons created per event: " << (fWLSEmissionEnergy / eV) / TotNbofEvents << " eV." << G4endl;

  G4cout << "Average number of photons absorbed by WLS2 per event: " << fWLS2AbsorptionCount / G4double(TotNbofEvents) << " " << G4endl;
  if(fWLS2AbsorptionCount > 0)
  {
    G4cout << " Average energy per photon: " << (fWLS2AbsorptionEnergy / eV) / fWLS2AbsorptionCount << " eV." << G4endl;
  }
  G4cout << "Average number of photons created by WLS2 per event: " << fWLS2EmissionCount / TotNbofEvents << G4endl;
  if(fWLS2EmissionCount > 0)
  {
    G4cout << " Average energy per photon: " << (fWLS2EmissionEnergy / eV) / fWLS2EmissionCount << " eV." << G4endl;
  }
  G4cout << "Average energy of WLS2 photons created per event: " << (fWLS2EmissionEnergy / eV) / TotNbofEvents << " eV." << G4endl;
  ...
  if(fParticle->GetParticleName() == "opticalphoton")
  {
    G4cout << "Unaccounted for:             " << std::setw(8) << fTotalSurface + fOpAbsorptionPrior - TotNbofEvents << G4endl;
  }
    ...
    if(fPolarized)
    {
      ...
      for(size_t i = 0; i < refl.size(); ++i)
      {
        if(refl[i] < min_val)
        {
          ...
          bin_width = histo_refl->axis().bin_upper_edge(i) - histo_refl->axis().bin_lower_edge(i);
      ...
      if(fPolarization == 0.0)
      {
        ...
        G4cout << " deg. Expected Brewster angle: " << (360. / CLHEP::twopi) * std::atan(rindex2 / rindex1) << " deg. " << G4endl;
      }
      ...
      G4cout << "Transmission goes to 0 at: " << min_angle << " +/- " << bin_width / 2. << " deg." << " Expected: " << (360. / CLHEP::twopi) * std::asin(rindex2 / rindex1) << " deg." << G4endl;
      ...
      {
        G4cout << "Not going to normalize transmission and reflection " << "histograms because bins are not the same." << G4endl;
      }
    }
  }
}
